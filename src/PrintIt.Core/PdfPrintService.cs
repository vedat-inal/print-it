...
namespace PrintIt.Core
{
    ...
    internal sealed class PdfPrintService : IPdfPrintService
    {
        ...
        public void Print(Stream pdfStream, string printerName, string pageRange = null, int numberOfCopies = 1, int paperSource = 0)
        {
            if (pdfStream == null)
            ...
            printDocument.DefaultPageSettings.PaperSource = new PaperSource() { RawKind = paperSource };
    ...
    {
        void Print(Stream pdfStream, string printerName, string pageRange = null, int numberOfCopies = 1, int paperSource = 0);
    }
}
